...
const filter = ["Flight", "Hotel", "Combo", "Cabs", "Special Offers"]

const BestOffers = () => {
  const [activeFilter, setActiveFilter] = React.useState("All Offers");
  const { offers, loading, error } = useLandingPage();
  ...
  return (
    <section className='relative bg-none pb-20'>
        <div className='max-w-7xl mx-auto px-4 bg-blue-600 py-10 rounded-xl'>
            <h2 className='text-2xl md:text-4xl font-bold mb-4 text-white'>Best Offers</h2>
            <div className='mb-4 space-y-2'>
                <button 
                onClick={() => changingFilter("All Offers")}
                className={`bg-white px-3 py-2 rounded-md  ml-2 ${activeFilter == "All Offers" ? "text-blue-600 font-semibold":""}`}>All Offers</button>
                {
                    filter.map((item, index) => (
                        <button
                        onClick={() => changingFilter(item)}
                        key={index} className={`bg-white px-3 py-2 rounded-md  ml-2 ${activeFilter == item ? "text-blue-600 font-semibold":""}`}>{item}</button>
                    ))
                }
            </div>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
                ...
                <div className='relative h-[400px] rounded-2xl overflow-hidden group cursor-pointer'>
                    <img 
                        src="/bestOffers_explore.png" 
                        alt="Explore More"
                        className='w-full h-full object-cover transition-transform duration-300 group-hover:scale-110'
                    />
                    <div className='absolute  bottom-0 bg-gradient-to-t from-[#5A9AA5] to-black/10 flex flex-col justify-end items-center pb-6 h-[50%]'>
